...
当前仓库可以直接复制到 DGX Spark 和 x86_64 RTX 主机，开始分阶段部署验证；
不能在未填写 `.env`、未准备地图和未锁定镜像时直接执行全链路
`docker compose up` 并宣称系统已经落地。
...
| x86_64 AWSIM + DGX Spark Autoware | `CONDITIONAL` | Compose 已具备，但真实镜像、地图、命令、网络和 ROS 契约待目标机验证 |
...
| G-01 | DGX Spark Linux、ARM64、内存、磁盘、Docker | `NOT-RUN` | 必须在目标 DGX Spark 执行 |
| G-02 | DGX GPU 容器 CUDA 运算 | `NOT-RUN` | 需要真实 `GPU_SMOKE_IMAGE` |
| G-03 | Autoware ARM64 镜像和业务健康探针 | `BLOCKED` | `.env.example` 仍是占位值 |
| G-04 | AWSIM x86_64 镜像和业务健康探针 | `BLOCKED` | `.env.example` 仍是占位值 |
| G-05 | 地图、车辆模型、sensor kit | `BLOCKED` | `data/maps` 当前为空，启动参数未锁定 |
